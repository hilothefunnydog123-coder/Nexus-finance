...
shortSig := not crashOff and score <= -minScore and regimeDn`,
  ...
  // ─── 3. ADAPTIVE REGIME-SWITCHING ───
  {
    ...
    propFirms: ['FTMO', 'Topstep', 'Apex', 'MyFundedFutures'],
    winTarget: 'Real MNQ 5-min: ~80% win · PF ~5 at the strict gate — VR loosened just enough for ~9–10 trades/mo', riskPerTrade: '0.5% risk-based',
    ...
    propNotes: 'WHAT THE REAL DATA SAID. The strict, fixed-target config — VR-gated breakout with the extension (0.5 ATR) and strong-close (0.7) quality filters, a 1.5R target and a 2-ATR stop — posted roughly 80% win / profit factor ~5 on a real MNQ 5-min sample, at ~6 trades/month. When I then loosened the breakout-quality filters to chase frequency, the real win rate collapsed to ~44% and it went break-even-to-losing. LESSON LEARNED: the extension + strong-close filters ARE the edge; do not touch them. So this build keeps every quality filter at its strict, proven value and loosens ONLY the regime gate (VR 1.4 → 1.2) — the one knob that adds trades without admitting low-quality breakouts — to lift frequency to about 9–10 a month. Expect to stay near the ~80% win / high-PF profile; the win rate lives in the quality filters, which are untouched. If you want even more trades, nudge VR down further (1.2 → 1.15 → 1.1) a step at a time and watch the win rate — that is the safe dial. The fixed target is the proven exit; the optional scale-out/trail and midday-skip toggles are OFF by default (they did not help in the real test). Honest note: my backtest simulator over-states momentum win rates, so I am NOT quoting a sim number — this is calibrated to YOUR real results, which is the only thing that has been right. Run it and send the trade list.',
