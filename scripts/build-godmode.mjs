...
minStr    = input.float(0.55, "Breakout bar closes in top/bottom × of range", step=0.05, minval=0, maxval=1, group="② Breakout", tooltip="Commitment filter: the breakout candle must close in the upper (long) / lower (short) part of its range, not a weak wick poke.")
extAtr    = input.float(0.2,  "Extend past channel (ATR ×)", step=0.05, group="② Breakout", tooltip="Price must push this far BEYOND the channel before entering — filters out false pokes that instantly reverse. The single biggest profit-factor lever; loosened to 0.2 to lift frequency to ~10/mo live while win rate / PF held out-of-sample.")
// ═══════════ ③ STOP / TARGET ═══════════
