...
console.log('  (FREQUENCY CHECK — sim punishes fades on WR/PF; we want ~15 trades/mo. RSI period is the lever.)')
console.log(row('  RSI(14)<25, morning 09:30-12:00',       mc({rsiLen:14,rsiLo:25,rsiHi:75,emaLen:200,slopeLkbk:20,winStart:0,winEnd:30})))
console.log(row('  RSI(5)<20, morning',                    mc({rsiLen:5,rsiLo:20,rsiHi:80,emaLen:200,slopeLkbk:20,winStart:0,winEnd:30})))
console.log(row('  RSI(3)<15, morning',                    mc({rsiLen:3,rsiLo:15,rsiHi:85,emaLen:200,slopeLkbk:20,winStart:0,winEnd:30})))
console.log(row('  RSI(2)<10 (Connors), morning',          mc({rsiLen:2,rsiLo:10,rsiHi:90,emaLen:200,slopeLkbk:20,winStart:0,winEnd:30})))
console.log(row('  RSI(2)<10, window to 13:00',            mc({rsiLen:2,rsiLo:10,rsiHi:90,emaLen:200,slopeLkbk:20,winStart:0,winEnd:42})))
console.log(row('  RSI(2)<10, full RTH',                   mc({rsiLen:2,rsiLo:10,rsiHi:90,emaLen:200,slopeLkbk:20,winStart:0,winEnd:77})))
console.log('  --- pushing toward ~15/mo (full RTH, RSI 2) ---')
console.log(row('  RSI(2)<15, full RTH',                   mc({rsiLen:2,rsiLo:15,rsiHi:85,emaLen:200,slopeLkbk:20,winStart:0,winEnd:77})))
console.log(row('  RSI(2)<20, full RTH',                   mc({rsiLen:2,rsiLo:20,rsiHi:80,emaLen:200,slopeLkbk:20,winStart:0,winEnd:77})))
console.log(row('  RSI(2)<15, full RTH, no reversal tick', mc({rsiLen:2,rsiLo:15,rsiHi:85,emaLen:200,slopeLkbk:20,winStart:0,winEnd:77,confirm:false})))
console.log(row('  RSI(3)<25, full RTH',                   mc({rsiLen:3,rsiLo:25,rsiHi:75,emaLen:200,slopeLkbk:20,winStart:0,winEnd:77})))
