...
function sigRegime(bars,P){
  const {reg=200,vrLag=5,vrLen=60,vrHi=1.1,vrLo=0.9,dc=20,extAtr=1.5,useRevert=false,
         strength=0,extReq=0,emaSlope=false}=P
  ...
  for(let i=Math.max(reg,vrLen,dc)+vrLag+1;i<bars.length;i++){
    ...
    if(trending){
      ...
      const closePos=(b.c-b.l)/rng                 // 1 = closed on high, 0 = on low
      const upStrong = closePos>=strength          // breakout bar closed with commitment
      const dnStrong = (1-closePos)>=strength
      const upExt = c[i] >= hh + extReq*a[i]        // pushed beyond the channel by k×ATR
      const dnExt = c[i] <= ll - extReq*a[i]
      const slopeOk = !emaSlope || eR[i]>eR[i-1]
      const slopeDn = !emaSlope || eR[i]<eR[i-1]
      if(c[i]>hh&&c[i]>eR[i]&&upStrong&&upExt&&slopeOk)s=1
      else if(c[i]<ll&&c[i]<eR[i]&&dnStrong&&dnExt&&slopeDn)s=-1
...
// What lifts PF: entry quality, not target placement. Stack filters one at a time.
const baseQ={ vrHi:1.25, vrLo:0.9, dc:30, slAtr:1.5, tpR:1.3, cooldown:3, useBE:false }
console.log('\n  Entry-quality stack (each filter added on top of the last):')
console.log(row('  base (VR 1.25, 1.3R target)',        mc(baseQ)))
console.log(row('  + breakout closes strong (≥0.6)',    mc({ ...baseQ, strength:0.6 })))
console.log(row('  + extends ≥0.25 ATR past channel',   mc({ ...baseQ, strength:0.6, extReq:0.25 })))
console.log(row('  + 200-EMA sloping with trade',       mc({ ...baseQ, strength:0.6, extReq:0.25, emaSlope:true })))
console.log(row('  + stricter VR gate (1.4)',           mc({ ...baseQ, vrHi:1.4, strength:0.6, extReq:0.25, emaSlope:true })))

console.log('\n  Grid → MAX PROFIT FACTOR, win ≥ 50%, ≥4 trades/mo, streak ≤ 8:')
let best=null,n=0
for(const vrHi of [1.25,1.35,1.5])
 for(const dc of [25,35,50])
  for(const slAtr of [1.2,1.5,2.0])
   for(const tpR of [1.0,1.3,1.6])
    for(const strength of [0.5,0.6,0.7])
     for(const extReq of [0,0.25,0.5])
      for(const emaSlope of [false,true]){
        n++
        const P={vrHi,vrLo:0.9,dc,slAtr,tpR,cooldown:3,useBE:false,strength,extReq,emaSlope,useRevert:false}
        const r=mc(P,6)
        if(r.perMonth>=4&&r.win>=0.5&&r.worst<=8&&(!best||r.pf>best.r.pf)) best={P,r}
      }
console.log(`  searched ${n} configs × 6 seeds`)
if(best){console.log('\n  ✅ HIGHEST PF (win ≥ 50%):');console.log('  '+JSON.stringify(best.P));console.log(row('  confirm (20 seeds)',mc(best.P,20)))}
else console.log('  nothing met the constraints')
console.log('')
