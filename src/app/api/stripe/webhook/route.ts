...
async function upsertSubscription(params: {
  ...
  if (!supabase) { console.warn('[webhook] Supabase not configured — skipping DB write'); return }
  ...
  try {
    switch (event.type) {
      ...
      case 'invoice.paid': {
        ...
        // Only send renewal email for recurring invoices (not the first one)
        const isRenewal = invAny.billing_reason === 'subscription_cycle'

        const sub    = await stripe.subscriptions.retrieve(subId)
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const subAny = sub as any
        const email  = await resolveEmail(invAny.customer as string, invAny.customer_email)
        ...
        if (isRenewal) {
          const periodEnd = subAny.current_period_end
            ? new Date(subAny.current_period_end * 1000).toISOString()
            : undefined
          await sendRenewalEmail(email, periodEnd)
        }
        break
      }
      ...
      case 'customer.subscription.deleted': {
        ...
        const accessUntil = subAny.current_period_end
          ? new Date(subAny.current_period_end * 1000).toISOString()
          : undefined
        await sendCancellationEmail(email, accessUntil)
        break
      }
    ...
    // Return 200 regardless so Stripe doesn't retry indefinitely
  }
