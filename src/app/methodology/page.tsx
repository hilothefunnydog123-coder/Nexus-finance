...
const SECTIONS = [
  ...
  { t: 'How the options figures work', b: 'The AI suggests a structure (a call or put), an approximate strike, breakeven and max loss, and draws the payoff at expiry. When available, we then pull the real contract nearest that strike from the live Polygon options chain — actual mid premium, implied volatility, delta/theta, open interest and breakeven (quotes can be up to ~15 minutes delayed). If live chain data isn’t available for a name, we fall back to a realized-volatility estimate and label it as such.' },
